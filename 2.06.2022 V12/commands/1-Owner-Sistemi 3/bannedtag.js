                   ...
                   await bannedTag.findOneAndUpdate({ guildID: message.guild.id }, { $push: { tags: args[1] } }, { upsert: true });
                    üyeler.map(x => {
                        ...
                        setTimeout(() => {
                            ...
                            x.roles.set(conf.yasaklıtagRole)
                        }, 1000)
                    })
                } else {
            ...
            if (args[0] == "kontrol") {
                ...
                res.taglar.forEach(x => {
                    let üye = message.guild.members.cache.filter(mems => {
                        return mems.user.username.includes(x) && !mems.roles.cache.has(conf.yasaklıtagRole)
                    }).map(x => x.id)
                    message.channel.send(`${x} tagı bulunup <@&${conf.yasaklıtagRole}> rolü olmayan ${üye.length} kişiye rolü veriyorum.`)
                    for (let i = 0; i < üye.length;i++) {
                        setTimeout(() => {
                            message.guild.members.cache.get(üye[i]).roles.set(conf.yasaklıtagRole)
                        }, (i + 1) * 1000)
                    }
                })
            }

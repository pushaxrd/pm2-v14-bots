...
module.exports = async (message) => {
  ...
  if (message.author.bot || !message.guild || !prefix || ayar.unregRoles.some(x => message.member.roles.cache.has(x)) || ayar.jailRole.some(x => message.member.roles.cache.has(x))) return;
  ...
  let komutLog = client.channels.cache.get(ayar.komutLog);
  ...
  if (cmd) {
    ...
    if(komutLog) komutLog.wsend(new MessageEmbed().setColor("#2f3136").setTimestamp().setFooter(settings.botDurum).setDescription(`${message.author} (\`${message.author.id}\`) tarafından ${message.channel} (\`${message.channel.id}\`) kanalında \`${prefix}${commandName}\` komutunu kullandı!\n\nKomut içeriği: \`${message.content}\`!`))
  }
};

...
module.exports = async (oldState, newState) => {
  ...
  const pubVeriler = await userParent.findOne({ guildID: oldState.guild.id, userID: oldState.id, parentID: cfg.publicParents });
...
async function saveDatas(user, channel, data) {
  if (cfg.staffs.some(x => user.member.roles.cache.has(x))) {
    if (channel.parent && cfg.publicParents.includes(channel.parentID)) {
    ...
    if (coinData && client.ranks.some(x => x.coin >= coinData.coin)) {
      ...
      if (newRank && Array.isArray(newRank.role) && !newRank.role.some(x => user.member.roles.cache.has(x)) || newRank && !Array.isArray(newRank.role) && !user.member.roles.cache.has(newRank.role)) {
        ...
        user.guild.channels.cache.get(cfg.rankLog).send(`${user.member.toString()} üyesi **${coinData.coin}** coin hedefine ulaştı ve **${Array.isArray(newRank.role) ? newRank.role.map(x => `${user.guild.roles.cache.get(x).name}`).join(", ") : `${user.guild.roles.cache.get(newRank.role).name}`}** rolü verildi! :tada: :tada:`);
      }
    }
  }
